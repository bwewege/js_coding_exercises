...
export const sumMultiples = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    runningTotal =
      runningTotal + (arr[i] % 3 === 0 || arr[i] % 5 === 0 ? arr[i] : 0);
  }
...
/**
 * This function will receive a string of characters and should
 * return true/false depending on whether it is a valid DNA string.
...
export const isValidDNA = (str) => {
  ...
  const compare = arrStr.filter((x) => !dna.includes(x));

  return compare.length === 0 ? true : false;
};
...
/**
 * This function should receive a number and return true/false depending
 * on whether it is a prime number or not. A prime number is a number that
 * can only be divided evenly by 1 and itself (for example, 7)
 * @param {Number} n
 * @returns {Boolean}
 */
 ...
 * with n items. The parameter "fill" should be used as the filler of the arrays.
